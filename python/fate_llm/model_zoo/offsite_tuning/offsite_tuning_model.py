...
def get_dropout_emulator_and_adapters(transformer_layers: nn.ModuleList, emulator_layer_num: int, adapter_top_layer_num: int, adapter_bottom_layer_num: int):
    ...
    LOGGER.info('take layer {} of the original model as the emulator'.format(t.Tensor(layer_idx) + bottom_idx))
...
class OffsiteTuningBaseModel(t.nn.Module):
    ...
    def get_submodel_weights(self) -> dict:
        ...
        for k, v in self.addition_param.items():
            addition_weights[k] = {k: v.detach().cpu().numpy() for k, v in v.state_dict().items()}
    ...
    def load_submodel_weights(self, submodel_weights: dict):
        ...
        for k, v in self.addition_param.items():
            if k not in submodel_weights:
                continue
            addition_weights = {k: t.tensor(v) for k, v in submodel_weights[k].items()}
