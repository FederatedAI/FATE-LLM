...
def get_llm_conf(params):
    embedding_model = LLMModelLoader(
        ...
        model_name_or_path=params['paths']['embedding_model_path']
    )

    dataset = LLMDatasetLoader(
        "flex_dataset",
        "FlexDataset",
        tokenizer_name_or_path=params['paths']['llm_pretrained_path'],
    ...
    training_args = FDKTTrainingArguments(
        ...
        use_cpu=params['training']['use_cpu'],
    ...
    inference_inst_conf = dict(
        ...
        kwargs=dict(
            ...
            model_name=params['paths']['llm_pretrained_path'],
            api_key="demo"
        )
    )
...
def get_slm_conf(params):
    slm_model = LLMModelLoader(
        "hf_model",
        "HFAutoModelForCausalLM",
        pretrained_model_name_or_path=params['paths']['slm_pretrained_path'],
    ...
    tokenizer = LLMDataFuncLoader(
        ...
        "get_tokenizer",
        tokenizer_name_or_path=params['paths']['slm_pretrained_path'],
        pad_token_id=50256
    )
    ...
    training_args = FDKTTrainingArguments(
        ...
        per_device_train_batch_size=params['training']['per_device_train_batch_size'],
        slm_generation_batch_size=params['training']['slm_generation_batch_size'],
        seq_num_for_single_category=params['training']['seq_num_for_single_category'],
        slm_generation_config=params['training']['slm_generation_config'],
    )
